...
/*
(Comment from C. Antollini)
THE PROBLEM:

when i try to use the ADOX 2.10 classes for an MFC Exe. Its compiling.
But when i compile the same classes in a MFC DLL . The below error is popping up.

d:\#arxbachi\arxdb\debug\msado15.tlh(2663) : error C2059: syntax error : 'constant'
d:\#arxbachi\arxdb\debug\msado15.tlh(2663) : error C2238: unexpected token(s) preceding ';'
d:\#arxbachi\arxdb\debug\msjro.tlh(48) : error C2011: 'ReplicaTypeEnum' : 'enum' type redefinition

i have done all the settings of using pre-compiled headers and other things.. but its
not compiling in the dll.
...
#ifdef UNDEFINED // Doesn't work with VC++ 2003
class CJetEngine
{
public:
...
public:
	...
	BOOL Open(LPCTSTR lpstrConnection = _T(""), LPCTSTR lpstrUserID = _T(""), LPCTSTR lpstrPassword = _T(""), CString* errMsg = NULL);
	_ConnectionPtr GetActiveConnection()
		{return m_pConnection;};
	BOOL Execute(LPCTSTR lpstrExec, CString* errMsg = NULL);
...
protected:
	void dump_com_error(_com_error &e, CString* errMsg = NULL);
...
{
public:
	BOOL Clone(CADORecordset& pRs, CString* errMsg = NULL);
	...
	void CancelUpdate();
	BOOL Update(CString* errMsg = NULL);
	void Edit();
	BOOL AddNew();
	BOOL AddNew(CADORecordBinding &pAdoRecordBinding, CString* errMsg = NULL);
	...
	BOOL IsOpen();
	void Close();
	BOOL Open(_ConnectionPtr mpdb, LPCTSTR lpstrExec = _T(""), int nOption = CADORecordset::openUnknown, CString* errMsg = NULL);
	BOOL Open(LPCTSTR lpstrExec = _T(""), int nOption = CADORecordset::openUnknown, CString* errMsg = NULL);
	BOOL OpenSchema(int nSchema, LPCTSTR SchemaID = _T(""), CString* errMsg = NULL);
	long GetFieldCount()
		{return m_pRecordset->Fields->GetCount();};
	BOOL GetFieldValue(LPCTSTR lpFieldName, int& nValue, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, int& nValue, CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, long& lValue, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, long& lValue, CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, unsigned long& ulValue, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, unsigned long& ulValue, CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, double& dbValue, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, double& dbValue, CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, CString& strValue, CString strDateFormat = _T(""), CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, CString& strValue, CString strDateFormat = _T(""), CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, COleDateTime& time, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, COleDateTime& time, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, bool& bValue, CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, bool& bValue, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, COleCurrency& cyValue, CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, COleCurrency& cyValue, CString* errMsg = NULL);
	BOOL GetFieldValue(int nIndex, _variant_t& vtValue, CString* errMsg = NULL);
	BOOL GetFieldValue(LPCTSTR lpFieldName, _variant_t& vtValue, CString* errMsg = NULL);

	BOOL IsFieldNull(LPCTSTR lpFieldName, CString* errMsg = NULL);
	BOOL IsFieldNull(int nIndex, CString* errMsg = NULL);
	BOOL IsFieldEmpty(LPCTSTR lpFieldName, CString* errMsg = NULL);
	BOOL IsFieldEmpty(int nIndex, CString* errMsg = NULL);
	BOOL IsEof()
	...
	BOOL GetFieldInfo(LPCTSTR lpFieldName, CADOFieldInfo* fldInfo, CString* errMsg = NULL);
	BOOL GetFieldInfo(int nIndex, CADOFieldInfo* fldInfo, CString* errMsg = NULL);
	BOOL AppendChunk(LPCTSTR lpFieldName, LPVOID lpData, UINT nBytes, CString* errMsg = NULL);
	BOOL AppendChunk(int nIndex, LPVOID lpData, UINT nBytes, CString* errMsg = NULL);

	BOOL GetChunk(LPCTSTR lpFieldName, CString& strValue, CString* errMsg = NULL);
	BOOL GetChunk(int nIndex, CString& strValue, CString* errMsg = NULL);

	BOOL GetChunk(LPCTSTR lpFieldName, LPVOID pData, CString* errMsg = NULL);
	BOOL GetChunk(int nIndex, LPVOID pData, CString* errMsg = NULL);
	...
	BOOL SetFilter(LPCTSTR strFilter, CString* errMsg = NULL);
	BOOL SetSort(LPCTSTR lpstrCriteria, CString* errMsg = NULL);
	BOOL SaveAsXML(LPCTSTR lpstrXMLFile, CString* errMsg = NULL);
	BOOL OpenXML(LPCTSTR lpstrXMLFile, CString* errMsg = NULL);
	BOOL Execute(CADOCommand* pCommand, CString* errMsg = NULL);
	BOOL Requery(CString* errMsg = NULL);
...
protected:
	BOOL PutFieldValue(LPCTSTR lpFieldName, _variant_t vtFld, CString* errMsg = NULL);
	BOOL PutFieldValue(_variant_t vtIndex, _variant_t vtFld, CString* errMsg = NULL);
	BOOL GetFieldInfo(FieldPtr pField, CADOFieldInfo* fldInfo, CString* errMsg = NULL);
	BOOL GetChunk(FieldPtr pField, CString& strValue, CString* errMsg = NULL);
	BOOL GetChunk(FieldPtr pField, LPVOID lpData, CString* errMsg = NULL);
	BOOL AppendChunk(FieldPtr pField, LPVOID lpData, UINT nBytes, CString* errMsg = NULL);
...
public:
	...
	BOOL SetValue(int nValue, CString* errMsg = NULL);
	BOOL SetValue(long lValue, CString* errMsg = NULL);
	BOOL SetValue(double dbValue, CString* errMsg = NULL);
	BOOL SetValue(CString strValue, CString* errMsg = NULL);
	BOOL SetValue(COleDateTime time, CString* errMsg = NULL);
	BOOL SetValue(_variant_t vtValue, CString* errMsg = NULL);
	BOOL GetValue(int& nValue, CString* errMsg = NULL);
	BOOL GetValue(long& lValue, CString* errMsg = NULL);
	BOOL GetValue(double& dbValue, CString* errMsg = NULL);
	BOOL GetValue(CString& strValue, CString strDateFormat = _T(""), CString* errMsg = NULL);
	BOOL GetValue(COleDateTime& time, CString* errMsg = NULL);
	BOOL GetValue(_variant_t& vtValue, CString* errMsg = NULL);
...
public:
	...
	BOOL AddParameter(CADOParameter* pAdoParameter, CString* errMsg = NULL);
	...
	BOOL AddParameter(CString strName, int nType, int nDirection, long lSize, _variant_t vtValue, int nPrecision = 0, int nScale = 0, CString* errMsg = NULL);
	CString GetText()
		{return m_strCommandText;};
	BOOL Execute(int nCommandType = typeCmdStoredProc, CString* errMsg = NULL);
	int GetRecordsAffected()
		{return m_nRecordsAffected;};
